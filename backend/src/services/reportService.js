...
function buildWeatherSummary(provincesData) {
  ...
  const avgTemp = temps.length ? (temps.reduce((a, b) => a + b, 0) / temps.length).toFixed(1) : 'N/D';
  ...
  const lines = provincesData.map(p =>
    ...
    `${p.current?.condition?.text?.substring(0, 28) || ''}  ` +
...
module.exports = { generateDailyReport, generateEmergencyReport, getLatestReport, getReports };
